...
#Check Normality with mixed model
...
# Variance models 
Model_Tot_Dist1 <- lme(tot_dist ~ Tank-1 + Sex, random = ~ 1| Fish_ID, data = Anxiety_Joined)
summary(Model_Tot_Dist1)
tab_model(Model_Tot_Dist1)
Model_Tot_Dist2 <- lme(tot_dist ~ Tank-1 + Sex, random = ~ 1| Fish_ID, weights = varIdent(form=~1|Tank), data = Anxiety_Joined)
...
model <- lmer(low_dur ~ Tank + Sex + (1 | Fish_ID), data = Old_Tanks)
summary(model)
...
tab_model(Model_freezing_dur1a)
...
# Model_latency2 <- lmer(sqrt(latency_high) ~ Tank + Sex + (1 | Fish_ID), data = Anxiety_Joined)
...
#Testing functions for new design of plots
```{r}
violin_plot_custom_hamza <- function(df,param,lab1,lab2) {
...
low_dur_violin <- violin_plot_custom_hamza(Anxiety_Joined,Anxiety_Joined$low_dur,"Time spent in low zone", "Time spent (seconds)")
