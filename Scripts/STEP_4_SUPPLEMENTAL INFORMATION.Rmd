...
Some data require trasformations
```{r}
#Total distance
Model_Tot_Dist_water <- lm(tot_dist ~ water_time, data = Anxiety_Joined)
...
#Low duration
Model_low_dur_water <- lm(low_dur ~ water_time, data = Anxiety_Joined)
...
#Mid duration
Model_mid_dur_water <- lm(mid_dur ~ water_time, data = Anxiety_Joined)
...
Model_high_dur_water <- lm(high_dur ~ water_time, data = Anxiety_Joined)
...
Model_freezing_dur_water <- lm(freezing_dur ~ water_time, data = Anxiety_Joined)
...
Model_freq_high_water <- lm(freq_high ~ water_time, data = Anxiety_Joined)
...
Model_latency_high_water <- lm(latency_high ~ water_time, data = Anxiety_Joined)
...
#TODO
model_lat <- lmer(sqrt(latency_high) ~ water_time + (1 |Fish_ID), data = Anxiety_Joined)

summary(model_lat)

hist(resid(model_lat))
```
